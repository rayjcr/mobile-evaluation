<template>
  <div class="wrapper">
        ...
        <div class="echarts-right flex_1">
          ...
          <div v-if="countInfo.evaluationRewardCountList">
            <div class="right-item" 
               v-for="(item,index) in countInfo.evaluationRewardCountList" :key="index">
              <div class="flex alignCenter" v-if="item.rewardType == 3">
                <span class="dot add-dot"></span>
                <span class="desc">加分: {{item.count}}次，共{{item.totalScore}}分</span>
              </div>
              <div class="flex alignCenter" v-else-if="item.rewardType == 4">
                <span class="dot reduce-dot"></span>
                <span class="desc">减分: {{item.count}}次，共{{item.totalScore}}分</span>
              </div>
              <div class="flex alignCenter" v-else-if="item.rewardType == 5">
                <span class="dot reward-dot"></span>
                <span class="desc">奖励勋章: {{item.count}}次，共{{item.totalScore}}枚</span>
              </div>
              <div class="flex alignCenter" v-else>
                <span class="dot deduct-dot"></span>
                <span class="desc">扣除勋章: {{item.count}}次，共{{item.totalScore}}枚</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <!--撤销评价记录？-->
    <v-confirm ref="confirm" :content="confirm_tip" @sure="undoRecord"></v-confirm>
    ...
    methods: {
      ...
      * */
      getClassRevokeTip(type) {
        this.confirm_tip = `撤销后该记录在教师端/学生端都不显示，${type == 2 ? '班级' : '学生'}增加/减少的分数或者勋章原路返回`;
      },
