...
<script>
  ...
  export default {
    ...
    activated() {
      Promise.all([this._initData(),this.getQuery(),this.getGroupInfo()]).then((result) => {}).catch((error) => {
    ...
    methods: {
      ...
      getGroupInfo() {
        let info = JSON.parse(sessionStorage.getItem('groupInfo'));

        this.groupName = info.groupName;
        this.selectedxsList = info.members;

        this.groupInfo = info;

        document.title = this.groupName || '编辑小组';
      },
      addStudent() {
        sessionStorage.setItem('groupName',this.groupName) // 存储小组名称
