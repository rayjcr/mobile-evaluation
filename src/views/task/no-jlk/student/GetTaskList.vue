<template>
  <ul class="tasklist">
    <li class="taskbox" v-for="(item,index) in list" :key="index" @click="jump(item.id,item.name,item.rewardType,item.taskType)">
...
<script>
  ...
  export default {
    ...
    methods: {
      jump(id,title,type,taskType) {
        let query = { id, title, type},path = '/stu/taskDetail';
        if(taskType == 2) {
          path = '/clockDetail';
          query = {
            id,
